...
def get_feature_columns(df: pd.DataFrame) -> list[str]:
    return [
        ...
        if (col.startswith("ch") and "_" in col) or col.startswith("asym_")
    ]
...
def run_representation_audit(representation: str, table_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    df = pd.read_csv(table_path)
    feature_cols = get_feature_columns(df)

    if len(feature_cols) == 0:
        raise RuntimeError(f"{representation}: no feature columns found.")
...
def aggregate_fold_metrics(fold_df: pd.DataFrame) -> pd.DataFrame:
    ...
    available = [col for col in metric_cols if col in fold_df.columns]
...
def main() -> None:
    ...
    for representation, path in FEATURE_TABLES.items():
        ...
        print(f"\nRunning asymmetry audit for representation: {representation}")
    ...
    fold_path = TABLES_DIR / "asymmetry_axis_audit_fold_metrics.csv"
    summary_path = TABLES_DIR / "asymmetry_axis_audit_summary.csv"
    best_path = TABLES_DIR / "asymmetry_axis_audit_best_models.csv"
    stability_path = TABLES_DIR / "asymmetry_axis_stability.csv"
    manifest_path = TABLES_DIR / "asymmetry_axis_audit_groupkfold_manifest.csv"
    metrics_path = METRICS_DIR / "asymmetry_axis_audit_summary.json"
    ...
    print("\nBest models with asymmetry:")
    print(best_df.to_string(index=False))

    print("\nAxis stability with asymmetry:")
